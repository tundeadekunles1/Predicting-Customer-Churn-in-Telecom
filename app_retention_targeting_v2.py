...
model_choice = st.sidebar.selectbox(
    "Model",
    options=[
        ("Logistic Regression", str(root / "data" / "models" / "logistic_regression.pkl")),
...
y_true = normalize_label(df_work[label_col]) if label_col is not None else None
...
if y_true is not None:
    if eval_on_holdout:
        ...
        idx_train, idx_test = train_test_split(
            idx_all, test_size=float(test_size), stratify=y_true.to_numpy(dtype=int), random_state=42
        )
        y_eval = y_true.to_numpy(dtype=int)[idx_test]
        p_eval = churn_prob[idx_test]
    else:
        y_eval = y_true.to_numpy(dtype=int)
        p_eval = churn_prob
    ...
    st.info("Select a churn label column in the sidebar to compute Baseline / Precision@k / Recall@k / Lift@k.")
